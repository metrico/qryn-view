...
export async function flushPerformanceQueue() {
    ...
    const metricsData = entries
        .map(
            (entry) =>
                `http_request,method=${entry.method},instance=${entry.instance},type=${entry.type},status=${entry.status},level=${entry.level ?? "info"} duration=${Math.round(entry.duration)} ${entry.timestamp * 1000000}`
        )
        .join("\n");
        ...
        streams: entries.map((entry) => ({
            ...
            values: [
                [
                    ...
                    `job="httpRequest" type=${entry.type} HTTP ${entry.method} ${entry.url} instance=${entry.instance} traceId=${entry.traceId} completed in ${entry.duration}ms with status ${entry.status} and duration ${entry.duration}`,
                ],
            ],
        })),
    });
    ...
    const tracesData = entries.map((entry) => ({
        ...
        tags: {
            ...
            "qryn-view.instance": entry.instance,
        },
        localEndpoint: {
...
export function requestPerformance(config: any): Promise<AxiosResponse> {
