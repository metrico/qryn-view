...
export const getTraceId = (trace:any) => trace.traceID;

export const getTraceSpans = (trace:any) => trace.spans;

const getTraceProcesses = (trace:any) => trace.processes;

const getSpanWithProcess = createSelector(
  (state:any) => state.span,
  (state:any) => state.processes,
  (span:any, processes:any) => ({
    ...span,
...
export const getTraceSpansAsMap = createSelector(getTraceSpans, (spans) =>
  spans.reduce((map:any, span:any) => map.set(getSpanId(span), span), new Map())
);

export const TREE_ROOT_ID:any = '__root__';
 ...
 */
export function getTraceSpanIdsAsTree(trace:any) {
  const nodesById = new Map(trace.spans.map((span:any) => [span.spanID, new TreeNode(span.spanID)]));
  const spansById = new Map(trace.spans.map((span:any) => [span.spanID, span]));
  ...
  const comparator = (nodeA:any, nodeB:any) => {
...
export const hydrateSpansWithProcesses = (trace:any) => {
  const spans:any = getTraceSpans(trace);
  const processes:any = getTraceProcesses(trace);

  return {
    ...trace,
    spans: spans.map((span:any) => getSpanWithProcess({ span, processes })),
  };
};
...
export const getTraceTimestamp = createSelector(getTraceSpans, (spans) =>
  spans.reduce(
    (prevTimestamp:any, span:any) => (prevTimestamp ? Math.min(prevTimestamp, getSpanTimestamp(span)) : getSpanTimestamp(span)),
    null
  )
);

export const getTraceDuration:any = createSelector(getTraceSpans, getTraceTimestamp, (spans, timestamp) =>
  spans.reduce(
    (prevDuration:any, span:any) =>
      prevDuration
...
export const getTraceEndTimestamp:any = createSelector(
  getTraceTimestamp,
  getTraceDuration,
...
export const getParentSpan = createSelector(
  ...
  (tree, spanMap) =>
    tree.children
      .map((node:any) => spanMap.get(node.value))
      .sort((spanA:any, spanB:any) => numberSortComparator(getSpanTimestamp(spanA), getSpanTimestamp(spanB)))[0]
);
...
export const getSpanDepthForTrace = createSelector(
  createSelector((state:any) => state.trace, getTraceSpanIdsAsTree),
  createSelector((state:any) => state.span, getSpanId),
...
export const formatDurationForUnit:any = createSelector(
...
export const formatDurationForTrace = createSelector(
  ({ duration }:{ duration:any }) => duration,
  createSelector(({ trace }:{ trace:any }) => trace, getDurationFormatterForTrace),
  (duration, formatter) => formatter(duration)
);

export const getSortedSpans = createSelector(
  ({ trace }:{ trace:any }) => trace,
...
const getTraceSpansByHierarchyPosition = createSelector(getTraceSpanIdsAsTree, (tree) => {
  ...
  let i = 0;
  tree.walk((spanID:any) => hierarchyPositionMap.set(spanID, i++));
...
export const getSpanHierarchySortPositionForTrace = createSelector(
  createSelector(({ trace }:{trace:any}) => trace, getTraceSpansByHierarchyPosition),
  ({ span }:{span:any}) => span,
...
export const omitCollapsedSpans = createSelector(
  ({ spans }:{spans:any}) => spans,
  createSelector(({ trace }:{trace:any}) => trace, getTraceSpanIdsAsTree),
  ({ collapsed }:{collapsed:any}) => collapsed,
  (spans, tree, collapse) => {
    const hiddenSpanIds = collapse.reduce((result:any, collapsedSpanId:any) => {
      tree.find(collapsedSpanId).walk((id:any) => id !== collapsedSpanId && result.add(id));
      return result;
    }, new Set());

    return hiddenSpanIds.size > 0 ? spans.filter((span:any) => !hiddenSpanIds.has(getSpanId(span))) : spans;
  }
);

export const DEFAULT_TICK_INTERVAL = 4;
export const DEFAULT_TICK_WIDTH = 3;
export const getTicksForTrace:any = createSelector(
  ({ trace }:{ trace:any }) => trace,
  ...
  (
    ...
    [...(interval + 1).keys()].map((num:number) => ({
...
export const enforceUniqueSpanIds = createSelector(
  /* istanbul ignore next */ (trace:any) => trace,
  getTraceSpans,
  /* istanbul ignore next */ (trace, spans) => {
    ...
    return {
      ...trace,
      spans: spans.reduce((result:any, span:any) => {
...
export const dropEmptyStartTimeSpans = createSelector(
  ...
  /* istanbul ignore next */ (trace, spans) => ({
    ...trace,
    spans: spans.filter((span:any) => !!getSpanTimestamp(span)),
  })
);
