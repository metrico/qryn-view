...
function adjustInterval(
    dynamicInterval:any, // number
    resolution:any, // number
    range:any // number   time range
) {
...
function getIntervalInfo(timespanMs:any) {
...
export default function adjustedStep(target:any, options:any) {

    const resolution = target.resolution || DEFAULT_RESOLUTION.value;
    const startNs = getTime(options.range.from);
    const endNs = getTime(options.range.to);
