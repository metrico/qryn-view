...
export const StatusBarCont = styled.div`
    ...
    background: ${({ theme }:{ theme:any }) => theme.widgetContainer};
`;
...
export const UrlCopyButton = styled.div`
    ...
    background: ${(props:any) => props.theme.buttonDefault};
    color: ${({ isActive, theme }:{ isActive:any; theme:any }) => (isActive ? "orange" : theme.textColor)};
    cursor: ${({ isActive }:{ isActive:any }) => (isActive ? "pointer" : "not-allowed")};
    text-overflow: ellipsis;
    transition: 0.2s all;
    margin-right:3px;
    span {
