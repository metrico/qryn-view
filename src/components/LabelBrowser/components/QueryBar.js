...
import { InputGroup, MobileTopQueryMenu, QueryBarContainer, SettingLabel } from "./styled";
/**Buttons */
...
export const QueryBar = (props) => {
          ...
          if (newLabels.length > 0) {
            if (prevMap.length > 0) {
              newLabels.forEach((l) => {
                ...
                if (labelFound) {
                  const pl = prevLabels.find((f) => f.name === l.name);
                  l = { ...pl };
                }
              });
            }
  ...
  const queryTypeRenderer = (type, traceSearch, querySearch) => {
    if (type === "traces") {
      return (
        <>
          <TracesSwitch value="traceId" onChange={switchTraceQueryType} />
  ...
  return (
    <div className={cx(maxWidth)} id={id}>
      <ThemeProvider theme={themes[theme]}>
        ...
        {queryTypeRenderer(
          dataSourceType,
          <TracesSearch {...props} onSearchChange={onTraceSearchChange} />,
          <QueryBarCont
            {...props}
            isSplit={isSplit}
            dataSourceType={dataSourceType}
...
export const MobileTopQueryMenuCont = (props) => {
  ...
  const panelQuery = useSelector((store) => store[name]);
  let isFluxChart = false;
  const handleChange = (e) => {
    isFluxChart = e.target.checked;
    console.log({ isFluxChart, e });
  };
  const fluxSwitcher = (type) => {
    if (type === "flux") {
      return (
        <div>
          <SettingLabel>Chart</SettingLabel>
          <Switch onChange={handleChange} name="isFluxChart" size="small" />;
        </div>
      );
    }
  };
