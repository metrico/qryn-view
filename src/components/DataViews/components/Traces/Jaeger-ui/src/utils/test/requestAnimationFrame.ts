...
const DEFAULT_ELAPSE:any = 0;

export default function requestAnimationFrame(callback:any) {
...
export function polyfill(target:any, msElapse:any = DEFAULT_ELAPSE) {
  ...
  if (!_target.requestAnimationFrame) {
    if (msElapse === DEFAULT_ELAPSE) {
      ...
      _target.requestAnimationFrame = (callback:any) => setTimeout(callback, msElapse);
    }
  }
